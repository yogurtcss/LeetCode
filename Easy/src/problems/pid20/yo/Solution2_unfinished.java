...
public class Solution2_unfinished {
    public boolean isValid(String s){
        ...
        ArrayList<Character> tempLeft = new ArrayList<>();
        ...
        for( int i=0,length=s.length(); i<length; i++ ){
            if( map.containsKey( s.charAt(i) ) ){
                ...
                if( tempLeft.size()-1>=0 && !map.get( tempLeft.remove(tempLeft.size()-1)).equals(s.charAt(i)) ) { //如果不成功
                    return false;
                }else if( tempLeft.size()==0 && i==length-1 ){ //此时i遍历到了“右括号”。
                    /* 如果(i==length-1)这是最后一个右括号，且tempLeft.size()==0没有左括号可以匹配
                    * 在外部已经排除了单字符的情况
                    *  */
                    return true;
                }else if( tempLeft.size()==0 ){ //如果还有多余的右括号，则直接false
                    return false;
                }
            }
        }

        return tempLeft.size()>0 ? false:true;
    };
}